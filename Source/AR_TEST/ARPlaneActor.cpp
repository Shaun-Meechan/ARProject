...
AARPlaneActor::AARPlaneActor()
{
	...
	PlanePolygonMeshComponentCollision = CreateDefaultSubobject<UProceduralMeshComponent>(TEXT("PlanePolygonMeshCollision"));

	// Take material from editor
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> MaterialAsset(TEXT("Material'/Game/HelloAR/Assets/Materials/ARPlane_Mat.ARPlane_Mat'"));
...
void AARPlaneActor::BeginPlay() 
{
	...
	PlanePolygonMeshComponentCollision->SetMaterial(0,PlaneMaterial);
}
...
void AARPlaneActor::Tick(float DeltaTime)
{
	...
	PlanePolygonMeshComponentCollision->SetWorldTransform(ARCorePlaneObject->GetLocalToWorldTransform());
	...
	case EARTrackingState::Tracking:
		...
		PlanePolygonMeshComponentCollision->SetVisibility(true);
		break;
	case EARTrackingState::NotTracking:
		PlanePolygonMeshComponent->SetVisibility(false);
		PlanePolygonMeshComponentCollision->SetVisibility(false);
		break;
...
void AARPlaneActor::UpdatePlanePolygonMesh()
{
	...
	PlanePolygonMeshComponentCollision->CreateMeshSection_LinearColor(0, PolygonMeshVertices, PolygonMeshIndices, PolygonMeshNormals, PolygonMeshUVs, PolygonMeshVertexColors, TArray<FProcMeshTangent>(), true);
	//Set the collision to allow the player to touch the map but block the missile (that is set in editor)
	PlanePolygonMeshComponentCollision->SetCollisionProfileName("OverlapAll");
}